...
<table><thead><tr><th>Session</th><th>Requests</th><th>CHR</th><th>Last model</th><th>Last seen</th></tr></thead><tbody id="session-rows"></tbody></table>
...
<div class="card">
<div class="detail-head"><button class="button" type="button" id="back-button">&#8592; Sessions</button><h2 id="detail-title" style="margin:0"></h2><span id="detail-chr" class="hint"></span></div>
...
const LIST_ROUTE="` + sessionsRoute + `",DETAIL_ROUTE="` + sessionDetailRoute + `",LIST_LIMIT=1000;
let auth=null;
...
function fmtRate(value){const rate=Number(value);return isFinite(rate)?(rate*100).toFixed(1)+"%":"\u2014"}
...
async function loadSessions(){status("Loading sessions\u2026");const body=document.getElementById("session-rows");clear(body);try{const payload=await request(LIST_ROUTE+"?limit="+LIST_LIMIT),sessions=(payload&&payload.sessions)||[];document.getElementById("sessions-empty").hidden=sessions.length>0;for(const session of sessions){const row=document.createElement("tr");row.tabIndex=0;row.setAttribute("role","button");const id=document.createElement("td"),idCode=document.createElement("code");idCode.textContent=shortID(session.session_id);idCode.title=session.session_id;id.append(idCode);const count=document.createElement("td");count.textContent=fmt(session.request_count);const chr=document.createElement("td");chr.textContent=fmtRate(session.cache_hit_rate);chr.title="Cache hit rate: cache read tokens / context tokens";const model=document.createElement("td");model.textContent=session.last_model||"\u2014";const seen=document.createElement("td");seen.textContent=fmtTime(session.last_seen);row.append(id,count,chr,model,seen);const open=()=>openSession(session.session_id);row.addEventListener("click",open);row.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();open()}});body.append(row)}status("")}catch(err){status(err.message,"error")}}
...
function hashSession(){const match=/^#session=(.+)$/.exec(location.hash||"");return match?decodeURIComponent(match[1]):""}
async function openSession(sessionID,push){status("Loading session\u2026");try{const payload=await request(DETAIL_ROUTE+"?session_id="+encodeURIComponent(sessionID));const title=document.getElementById("detail-title");clear(title);const code=document.createElement("code");code.textContent=payload.session_id;title.append(code);document.getElementById("detail-chr").textContent="CHR "+fmtRate(payload.cache_hit_rate);renderChart(payload);show("detail-view");if(push!==false)history.pushState({sessionID},"","#session="+encodeURIComponent(sessionID));status("")}catch(err){status(err.message,"error")}}
function goBack(){if(history.state&&history.state.sessionID){history.back();return}history.replaceState(null,"",location.pathname+location.search);show("sessions-view");status("")}
document.getElementById("back-button").addEventListener("click",goBack);
window.addEventListener("popstate",()=>{const sessionID=(history.state&&history.state.sessionID)||hashSession();if(sessionID){openSession(sessionID,false);return}show("sessions-view");status("")});
try{auth=loadAuth()}catch(err){document.getElementById("auth-block").classList.add("show");status(err.message,"error");return}
loadSessions().then(()=>{const initial=hashSession();if(initial){history.replaceState({sessionID:initial},"",location.hash);openSession(initial,false)}});
})();
</script>
</body>
</html>
